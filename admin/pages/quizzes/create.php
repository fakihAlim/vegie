<?php
/**
 * Quizzes Management - Create Manual Quiz
 * LovingHarmony Admin Panel
 */
$pageTitle = 'Tambah Kuis Nutrisi';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        $_SESSION['flash_success'] = 'Kuis manual berhasil ditambahkan! 🧩';
...
<div class="app-layout">
    ...
    <main class="main-content">
        <div class="top-bar">
            <span class="page-title">🧩 Tambah Kuis Nutrisi</span>
            <div class="user-menu">
                    ...
                    <form method="POST" action="">
                        <div class="form-group">
                            ...
                            <textarea name="question" id="question" rows="3" class="form-control" 
                                      placeholder="Contoh: Manakah di bawah ini yang merupakan sumber Vitamin B12 terbaik untuk vegan?" required><?= htmlspecialchars($_POST['question'] ?? '') ?></textarea>
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" name="option_c" id="option_c" class="form-control" 
                                       ...
                                       value="<?= htmlspecialchars($_POST['option_c'] ?? '') ?>" required>
                            </div>
                            <div class="form-group">
                                ...
                                <input type="text" name="option_d" id="option_d" class="form-control" 
                                       ...
                                       value="<?= htmlspecialchars($_POST['option_d'] ?? '') ?>" required>
                            </div>
                        </div>
                        ...
                        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px;">
                            <div class="form-group">
                                ...
                                <select name="correct_answer" id="correct_answer" class="form-control" required>
                                    ...
                                    <option value="a" <?= (isset($_POST['correct_answer']) && $_POST['correct_answer'] === 'a') ? 'selected' : '' ?>>A</option>
                                    <option value="b" <?= (isset($_POST['correct_answer']) && $_POST['correct_answer'] === 'b') ? 'selected' : '' ?>>B</option>
                                    <option value="c" <?= (isset($_POST['correct_answer']) && $_POST['correct_answer'] === 'c') ? 'selected' : '' ?>>C</option>
                                    <option value="d" <?= (isset($_POST['correct_answer']) && $_POST['correct_answer'] === 'd') ? 'selected' : '' ?>>D</option>
                                </select>
                            </div>
                            <div class="form-group">
                                <label for="points">Poin Alokasi</label>
                                <input type="number" name="points" id="points" class="form-control" min="5" max="1000" step="5"
                                       value="<?= htmlspecialchars($_POST['points'] ?? '50') ?>">
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="explanation" id="explanation" rows="3" class="form-control" 
                                      placeholder="Berikan penjelasan ilmiah kenapa jawaban tersebut benar..." required><?= htmlspecialchars($_POST['explanation'] ?? '') ?></textarea>
                        ...
                        <div class="form-group" style="display: flex; align-items: center; gap: 8px; margin-top: 16px;">
                            ...
                            <label for="is_active" style="margin: 0; font-weight: bold; cursor: pointer;">Aktifkan Langsung (Muncul di Aplikasi)</label>
                        </div>

                        <div class="form-actions" style="margin-top: 28px;">
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-check-circle"></i> Simpan Kuis
                            </button>
